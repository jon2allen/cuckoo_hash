...
bool insert(CuckooTable *t, ModelEntry *entry) {
  ...
  for (int i = 0; i < STASH_SIZE; i++) {
    if (t->stash[i] == NULL) {
      ...
      t->count++;
      if (verbose) {
        printf("Inserted '%s' into stash at index %d\n", current->model_name,
               i);
      }
      return true;
    }
...
int main(int argc, char *argv[]) {
  ...
  for (int r = 0; r < NUM_RUNS; r++) {
    ...
    total_lf += (float)t.count / (TOTAL_SLOTS + STASH_SIZE);
    all_displacements += t.total_displacements;
  }
  printf("Cuckoo with Stash: AvgTime=%.8f, AvgLF=%.4f, AvgDisp=%.2f\n",
